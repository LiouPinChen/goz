...
func Test_Example_Isolate(t *testing.T) {
	...
	isoCli := goz.NewClient() // goz.NewClient() // goz.NewIsolateClient()

	// 測試第一隻 API 在隔離狀態的的回應
	responseFormIsolated1, err := isoCli.Get("http://172.17.0.1:9000/server/msg1")
	if err != nil {
		log.Fatalln(err)
	}

	tmp , _ := responseFormIsolated1.GetBody()
